...
/*** TO BE DONE 5.0 START ***/
	...
	if(pthread_mutex_unlock(&cookie_mutex)!=0)
		fail_errno("UNLOCK ERROR in get_new_UID");
...
/*** TO BE DONE 5.0 START ***/
	if( gmtime_r(&now_t, &now_tm) == NULL )
		fail_errno("ERROR of gmtime_r in send_response");
