	...
	while (tmp) {
		...
		if (*tmp=='<') {
			...
			if (*tmp=='$') {
				...
				tmp= getenv(tmp+1); 
				if(tmp == NULL){
					fprintf(stderr, "Parsing error: error with variable\n");
					goto fail;
				}
...
check_t check_redirections(const line_t * const l)
{
	...
	for(int a=0; a<l->n_commands; ++a){		
		if(a > 0 && l->commands[a]->in_pathname){
			fprintf(stderr, "Only the first command of a line can have input-redirection\n");
			return CHECK_FAILED;			
		}

		if(a < l->n_commands-1 && l->commands[a]->out_pathname){
...
check_t check_cd(const line_t * const l)
{
	...
	for(int i = 0; i<l->n_commands; ++i){
		if(strcmp(CD, l->commands[i]->args[0])==0){
			if(l->n_commands != 1 || l->commands[i]->n_args != 2 || l->commands[i]->in_pathname || l->commands[i]->out_pathname){
	...
	*/
	...
	for(int i = 0; i<l->n_commands; ++i){
		if(strcmp(CD, l->commands[i]->args[0])==0){
			if(l->n_commands != 1 ){
				fprintf(stderr, "ERROR parsing: there are more commands besides cd\n");
				valid= CHECK_FAILED;
			}
				
			if(l->commands[i]->n_args != 2){
				fprintf(stderr, "ERROR parsing: the command has more than 2 arguments\n");
				valid= CHECK_FAILED;
			}
				
			if(l->commands[i]->in_pathname){
				fprintf(stderr, "ERROR parsing: the command has I redirection\n");
				valid= CHECK_FAILED;
			}
				
			if(l->commands[i]->out_pathname){
				fprintf(stderr, "ERROR parsing: the command has O redirection\n");
				valid= CHECK_FAILED;
			}
				
			if(valid == CHECK_FAILED)
				return CHECK_FAILED;
		}
	}
	/*** TO BE DONE END ***/
...
void wait_for_children()
{
	...
	while(pid_child != -1){					
		if(WIFEXITED(status) && WEXITSTATUS(status)!=0)
			fprintf(stderr, "ERROR with a children process: %lld exited with status: %d\n", (long long int)pid_child, WEXITSTATUS(status));
			
			
		if(WIFSIGNALED(status))
			fprintf(stderr, "ERROR with a children process: %lld was terminated by a signal %d\n)", (long long int)pid_child, WTERMSIG(status));
...
void redirect(int from_fd, int to_fd)	
{
	...
	if(from_fd != NO_REDIR){
		if( dup2(from_fd, to_fd) == -1)	
			fatal_errno("ERROR dup");
		
		if(close(from_fd)== -1)	
			fatal_errno("ERROR close from_fd");
	}
	...
	*/
	...
	if(p == 0){
		...
		if ( execvp(c->args[0], c->args) == -1)
				fatal_errno("ERROR with command in execvp");
	}
...
void change_current_directory(char *newdir)
{
	...
	/*** TO BE DONE START ***/
 	...
 	if( chdir(newdir) == -1 )
 		fatal_errno("ERROR change the working directory");
...
void execute_line(const line_t * const l)
{
	...
	for(int a=0; a<l->n_commands; ++a) {
		...
		if (c->in_pathname) {
			...
			if (curr_stdin == -1) {
				perror("ERROR open file for stdin");	
				return;		
			}
		...
		if (c->out_pathname) {
			...
			if (curr_stdout == -1) {
				perror("ERROR open file for stdout");
				return;
			}
			...
			/*
			if( pipe(fds)!=0 ) 
				fatal_errno("ERROR pipe");
				
			if (fcntl(fds[0], F_SETFD, FD_CLOEXEC)== -1)
				fatal_errno("ERROR fcntl on fds[0]");
				
			if (fcntl(fds[1], F_SETFD, FD_CLOEXEC)== -1)
				fatal_errno("ERROR fcntl on fds[1]");
			*/
