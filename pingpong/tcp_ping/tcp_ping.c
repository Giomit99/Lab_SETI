...
/*** TO BE DONE START ***/
	if(clock_gettime(CLOCK_TYPE, &send_time) != 0) 
		fail_errno("Problem with the clock_gettime");
...
/*** TO BE DONE START ***/
	if(blocking_write_all(tcp_socket, message, msg_size) == -1)
		fail_errno("ERROR blocking_write_all in do_ping");

	sent_bytes= msg_size;
/*** TO BE DONE END ***/

    /*** Receive answer through the socket (blocking) ***/
	for (offset = 0; (offset + (recv_bytes = recv(tcp_socket, offset + rec_buffer, sent_bytes - offset, MSG_WAITALL))) < msg_size; offset += recv_bytes) {
		debug(" ... received %zd bytes back\n", recv_bytes);
		if (recv_bytes < 0)
			fail_errno("Problem with receiving");
	}
...
/*** TO BE DONE START ***/
	...
	if(gai_rv < 0) 
		fail_errno("Problem with the system call getaddrinfo into prepare_udp_socket");
/*** TO BE DONE END ***/
...
/*** TO BE DONE START ***/
	...
	if(tcp_socket < 0) 	
		fail_errno("TPC socket not open");
		
	if(connect(tcp_socket, server_addrinfo->ai_addr, server_addrinfo->ai_addrlen) < 0) 
		fail_errno("TPC Ping cannot connect to server");
...
/*** TO BE DONE START ***/
	if(strncmp(answer, "OK ", 2)) 
		fail_errno("The answer is different from OK");
