...
/*** TO BE DONE START 	***/
		...
		if(sent_bytes == -1) 
			fail_errno("Problem with sending");
...
/*** TO BE DONE START ***/
	if(fcntl(ping_socket, F_SETFL, O_NONBLOCK) == -1) 
		fail_errno("Problem with the system call fcntl");
...
/*** TO BE DONE START ***/
	...
	if(connect(ping_socket, pong_addrinfo->ai_addr, pong_addrinfo->ai_addrlen) == -1) 
		fail_errno("UDP Ping cannot connect to server");
...
/*** TO BE DONE START ***/
	...
	if(gai_rv < 0)
		fail_errno("Problem with system call getaddrinfo into main");
...
/*** TO BE DONE START ***/
	...
	if(ask_socket < 0) 
		fail_errno("UDP socket not open");
